...
@php
    ...
    $buildUrl = function (array $override = [], array $remove = []) {
        ...
        $q['page'] = 1;
        return route('supervisor.dashboard') . '?' . http_build_query($q);
    };
    ...
    $isFiltered = request()->hasAny(['date_from', 'date_to', 'type', 'per_page']);
...
<div class="md:hidden px-5 py-6">
    ...
    <div class="mb-5">
        <div class="flex items-start justify-between gap-3">
            ...
            <a href="{{ route('supervisor.team.index') }}"
                class="shrink-0 inline-flex items-center gap-1.5 bg-blue-600 text-white text-xs font-bold px-3 py-2 rounded-xl shadow-sm active:scale-95 transition">
    ...
    <div class="mb-5">
        ...
        {{-- FILTER FORM: range tanggal + per halaman --}}
        <form method="GET" action="{{ route('supervisor.dashboard') }}" id="filterFormMobile">
            ...
            <select name="per_page" onchange="this.form.submit()" class="w-full border border-gray-200 rounded-lg pl-2 pr-6 py-2 text-[11px] text-gray-700 bg-white focus:ring-1 focus:ring-blue-500 focus:border-blue-500 outline-none">
                @foreach([10, 20, 50, 100] as $opt)
                    <option value="{{ $opt }}" @selected($perPage == $opt)>{{ $opt }}/halaman</option>
                @endforeach
            </select>
        </form>
    </div>
    ...
    @else
        <div class="text-center py-16">
            ...
            <p class="text-xs text-gray-400 mt-1">Aktivitas terbaru tim akan muncul di sini</p>
        </div>
    @endif
</div>
            ...
            <div class="bg-white rounded-[2rem] shadow-sm border border-gray-100 overflow-hidden">
                ...
                <div class="px-8 pt-8 pb-5">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <a href="{{ route('supervisor.team.index') }}"
                            class="inline-flex items-center gap-2 bg-blue-600 text-white text-xs font-bold px-4 py-2.5 rounded-xl shadow-sm hover:bg-blue-700 transition">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"></path></svg>
                            Daftar Karyawan
                        </a>
                    </div>
                    ...
                    {{-- FILTER FORM (DESKTOP) --}}
                    <form method="GET" action="{{ route('supervisor.dashboard') }}" id="filterFormDesktop" class="filter-form space-y-3">
                        <input type="hidden" name="type" value="{{ $reqType }}">
                        <input type="hidden" name="preset" value="{{ $reqPreset }}">
                        ...
                        {{-- Row 2: Per halaman + Reset (sebaris) --}}
                        <div class="flex items-center gap-3 flex-wrap">
                            ...
                            @if($isFiltered)
                                <a href="{{ route('supervisor.dashboard') }}" class="text-xs font-bold text-gray-400 hover:text-red-500 transition shrink-0 ml-auto">Reset Filter</a>
                            @endif
                        </div>
                    </form>
                </div>
                ...
                @if($activities->count() > 0)
                    <div class="px-8 pb-5 space-y-3" id="activityFeedDesktop">
                        @foreach($activities as $activity)
                            @include('hrd.partials._activity_item_desktop', [
                                'activity' => $activity,
                                'showUserRoute' => route('supervisor.show.sales', $activity['user']->id),
                            ])
                        @endforeach
                    </div>
