...
@section('content')
    <div class="px-5 md:px-8 py-6 md:py-8">
        ...
        <form action="{{ route('it.roles.update', $role->id) }}" method="POST" class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            @csrf
            @method('PUT')
            ...
            <div class="mb-5">
                ...
                <input type="text" name="dashboard_url" required value="{{ old('dashboard_url', $role->dashboard_url) }}"
                    class="w-full border border-gray-300 rounded-xl p-3 text-sm focus:ring-2 focus:ring-blue-500 outline-none">
            ...
            <div class="mb-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-3 bg-gray-50 p-4 rounded-xl border border-gray-100">
                    @foreach($permissions as $code => $label)
                        @php
                            $isChecked = is_array($role->permissions) && in_array($code, $role->permissions);
                            if(old('_token')) { // If validation failed, use old input
                                $isChecked = is_array(old('permissions')) && in_array($code, old('permissions'));
                            }
                        @endphp
                ...
                @if($role->slug === 'it' || (is_array($role->permissions) && in_array('all', $role->permissions)))
                    <p class="text-xs text-purple-600 font-bold mt-2">Role ini memiliki "Akses Penuh". Pilihan di atas dapat diabaikan.</p>
                @endif
            </div>
            ...
            <div class="mb-8 p-4 bg-purple-50 rounded-xl border border-purple-100">
                <label class="flex items-center space-x-3 cursor-pointer">
                    <input type="checkbox" name="hrd_can_create" value="1" 
                        {{ old('hrd_can_create', $role->hrd_can_create) ? 'checked' : '' }}
            ...
            <div class="pt-4 border-t border-gray-100 flex gap-3">
                <button type="submit" class="bg-blue-600 text-white px-6 py-3 rounded-xl font-bold hover:bg-blue-700 shadow-sm">Simpan Perubahan</button>
