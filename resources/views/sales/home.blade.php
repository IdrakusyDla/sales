...
@section('content')
    {{-- ========================================== --}}
    {{-- TAMPILAN MOBILE (< 768px): KODE ASLI UTUH --}} {{--==========================================--}} <div
        class="md:hidden">
        {{-- 1. HEADER PROFIL (Simpel) --}}
        <div
            class="bg-blue-600 text-white p-6 md:p-8 rounded-b-3xl md:rounded-b-none shadow-lg mb-6 relative overflow-hidden">
            <div
                class="absolute top-0 right-0 w-32 h-32 bg-white opacity-10 rounded-full -mr-10 -mt-10 pointer-events-none">
            </div>
            ...
            @else
                <div class="px-5 md:px-8 mb-6">
                    <div class="bg-yellow-50 border-2 border-yellow-200 text-yellow-800 p-4 rounded-2xl text-center">
                        ...
                        <p class="text-xs opacity-80">Anda masih memiliki
                            {{ $latestLog->visits()->where('status', 'pending')->count() }}
                            kunjungan yang belum dilaporkan
                        </p>
                    </div>
                </div>
            @endif
        ...
        <div class="px-5 md:px-8 pb-24 md:pb-8">
