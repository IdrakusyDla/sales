    ...
    <div class="hidden md:block px-8 py-8 h-full bg-slate-50/50">
        {{-- HEADER --}}
        <div class="mb-8">
            <h1 class="text-3xl font-extrabold text-gray-800 tracking-tight mb-2">Riwayat Aktivitas</h1>
            <p class="text-gray-500 font-medium">Lihat semua absensi dan kunjungan Anda secara detail</p>
        </div>

        {{-- FILTER TANGGAL --}}
        <div class="bg-white rounded-[2rem] shadow-sm border border-gray-100 p-8 mb-8">
            <form method="GET" action="{{ route('sales.history') }}" class="flex items-end gap-6">
                <div class="flex-1">
                ...
                <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white px-10 py-4 rounded-xl font-bold transition-colors shadow-md shadow-blue-600/20 active:scale-95 flex items-center gap-2 h-[58px]">
        ...
        <div class="grid grid-cols-3 gap-6">
            @forelse($dailyLogs as $log)
                <div class="relative bg-white rounded-2xl shadow-sm border border-gray-100 p-4 hover:shadow-md transition h-full flex flex-col justify-between group">
                    ...
                    <div class="relative z-20 pointer-events-none">
                        {{-- Make sure content that shouldn't be clicked doesn't block the link, EXCEPT for the buttons below --}}
                            ...
                            <div class="mt-3 pt-3 border-t border-gray-100">
                                ...
                                @if(!$log->hasEnded() || \Carbon\Carbon::today()->lte(\App\Models\Expense::calculateDeadline($log->date)))
                                    <a href="{{ route('sales.reimburse.form', $log->id) }}"
                                        class="relative z-30 block mt-3 text-center bg-blue-50 text-blue-600 py-2 rounded-lg text-xs font-bold hover:bg-blue-100 transition-colors">
            ...
            @empty
                <div class="col-span-3 text-center py-20 bg-white rounded-3xl border border-gray-100 shadow-sm opacity-60">
                    ...
                    <p class="text-xl font-bold text-gray-500 mb-2">Belum Ada Riwayat Aktivitas</p>
                    <p class="text-gray-400">Pilih rentang tanggal yang berbeda atau lakukan absensi hari ini.</p>
                </div>
            @endforelse
        </div>
    </div>
