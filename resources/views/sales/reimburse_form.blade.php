...
@section('content')
    <div class="px-5 md:px-8 py-6 md:py-8">
        ...
        <div class="mb-5 md:mb-8">
            <h1 class="text-2xl md:text-3xl font-bold md:font-extrabold text-gray-800 md:tracking-tight mb-2">
                Tambah Pengeluaran
            </h1>
            <p class="text-sm text-gray-600 md:text-gray-500 md:font-medium">
                Catat pengeluaran untuk tanggal:
                {{ \Carbon\Carbon::parse($dailyLog->date)->format('d M Y') }}
            </p>

            @if($deadline)
                <div class="mt-3 inline-flex items-center gap-2 bg-yellow-50 border border-yellow-200 rounded-full px-3 py-1.5">
        ...
        @if($expenses->count() > 0)
            ...
            <div class="mb-6 md:mb-8">
                <div class="flex items-center gap-2 mb-3 md:mb-4">
                    <h2 class="text-sm font-bold text-gray-700 md:text-base md:text-gray-800">Pengeluaran Tercatat</h2>
                    <span class="text-xs font-bold text-gray-500 bg-gray-100 rounded-full px-2 py-0.5">{{ $expenses->count() }}</span>
                </div>

                {{-- TOTAL SUMMARY --}}
                <div class="flex items-center justify-between gap-3 bg-gradient-to-r from-green-500 to-emerald-600 rounded-2xl md:rounded-[2rem] p-4 md:p-5 mb-3 md:mb-5 shadow-sm shadow-green-600/20">
                    <div class="flex items-center gap-3 md:gap-4 min-w-0">
                        <span class="w-10 h-10 md:w-12 md:h-12 rounded-xl md:rounded-2xl bg-white/20 flex items-center justify-center shrink-0">
                            <svg class="w-5 h-5 md:w-6 md:h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z"></path>
                            </svg>
                        </span>
                        <div class="min-w-0">
                            <p class="text-xs md:text-sm text-green-50 font-medium leading-tight">Total Pengeluaran</p>
                            <p class="text-xs text-green-100/80 leading-tight mt-0.5">{{ $expenses->count() }} item tercatat</p>
                        </div>
                    </div>
                    <p class="text-xl md:text-2xl font-extrabold text-white tracking-tight shrink-0 whitespace-nowrap">Rp {{ number_format($totalAmount, 0, ',', '.') }}</p>
                </div>

                <div class="space-y-2 md:grid md:grid-cols-3 md:gap-4 md:space-y-0">
                    @foreach($expenses as $expense)
                        <div class="bg-gray-50 rounded-xl p-3 flex items-center justify-between gap-3 md:bg-white md:rounded-2xl md:shadow-sm md:border md:border-gray-100 md:p-4 md:hover:shadow-md md:transition">
                            <div class="flex items-center gap-2 min-w-0">
                                ...
                                @endif
                                <div class="min-w-0">
                                    <p class="font-bold text-sm text-gray-800">{{ $typeLabels[$expense->type] ?? ucfirst($expense->type) }}</p>
                                    @if($expense->note)
                                        <p class="text-xs text-gray-500 truncate">{{ $expense->note }}</p>
                                    @endif
                                </div>
                            </div>
                            <p class="font-bold text-sm text-green-600 shrink-0 whitespace-nowrap md:text-base">Rp {{ number_format($expense->amount, 0, ',', '.') }}</p>
                        </div>
                    @endforeach
                </div>
            </div>
        ...
        {{-- FORM --}}
        <form action="{{ route('sales.reimburse.store', $dailyLog->id) }}" method="POST" id="form-reimburse" class="card-form">
            @csrf
            ...
            <div class="md:grid md:grid-cols-12 md:gap-6 md:items-start">
                ...
                {{-- ===== KOLOM KIRI: DETAIL PENGELUARAN ===== --}}
                <div class="md:col-span-7 space-y-4 md:space-y-6">

                    {{-- JENIS PENGELUARAN --}}
                    <div class="md:bg-white md:rounded-[2rem] md:shadow-sm md:border md:border-gray-100 md:p-6">
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-3 gap-2 md:gap-3">
                            <label class="cursor-pointer">
                                ...
                                <div
                                    class="border-2 border-gray-200 rounded-xl p-3 peer-checked:border-blue-500 peer-checked:bg-blue-50 transition md:p-4 md:hover:border-blue-300">
                            ...
                            <label class="cursor-pointer">
                                ...
                                <div
                                    class="border-2 border-gray-200 rounded-xl p-3 peer-checked:border-purple-500 peer-checked:bg-purple-50 transition md:p-4 md:hover:border-purple-300">
                                    <div class="flex items-center gap-2">
                                        <span class="w-8 h-8 rounded-full bg-purple-100 flex items-center justify-center shrink-0">
                            ...
                            <label class="cursor-pointer">
                                ...
                                <div
                                    class="border-2 border-gray-200 rounded-xl p-3 peer-checked:border-green-500 peer-checked:bg-green-50 transition md:p-4 md:hover:border-green-300">
                                    <div class="flex items-center gap-2">
                                        <span class="w-8 h-8 rounded-full bg-green-100 flex items-center justify-center shrink-0">
                                            <svg class="w-4 h-4 text-green-500" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="-20 -20 440 440" xml:space="preserve" fill="currentColor"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <g> <g> <path d="M295.225,142.697c-9.9-44.668-19.801-89.336-29.707-134.003c-16.718,0-33.435,0-50.15,0 c2.389,44.668,4.781,89.336,7.172,134.003H295.225z"></path> <path d="M226.354,214.003c3.145,58.703,6.286,117.404,9.426,176.107c38.094,0,76.188,0,114.281,0 c-13.014-58.702-26.021-117.404-39.029-176.107H226.354z"></path> <path d="M183.435,8.694c-16.716,0-33.434,0-50.149,0c-9.902,44.667-19.798,89.335-29.698,134.003h72.682 C178.656,98.029,181.043,53.361,183.435,8.694z"></path> <path d="M48.742,390.11c38.096,0,76.188,0,114.281,0c3.152-58.702,6.293-117.404,9.43-176.107H87.785 C74.775,272.706,61.763,331.409,48.742,390.11z"></path> <path d="M394.176,161.212H4.628c-2.556,0-4.628,2.072-4.628,4.628v25.02c0,2.556,2.072,4.628,4.628,4.628h25.048v37.476 c0,2.556,2.071,4.629,4.627,4.629h24.996c2.117,0,3.964-1.438,4.484-3.488l9.818-38.615h251.602l9.816,38.615 c0.52,2.052,2.369,3.488,4.486,3.488h24.992c2.559,0,4.629-2.073,4.629-4.629v-37.476h25.049c2.557,0,4.629-2.072,4.629-4.628 v-25.02C398.805,163.284,396.732,161.212,394.176,161.212z"></path> </g> </g> </g></svg>
                                        </span>
                            ...
                            <label class="cursor-pointer">
                                ...
                                <div
                                    class="border-2 border-gray-200 rounded-xl p-3 peer-checked:border-orange-500 peer-checked:bg-orange-50 transition md:p-4 md:hover:border-orange-300">
                                    <div class="flex items-center gap-2">
                                        <span class="w-8 h-8 rounded-full bg-orange-100 flex items-center justify-center shrink-0">
                                            <svg class="w-5 h-5 text-orange-500" fill="currentColor" viewBox="0 -3.6 30.859 30.859" xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <path id="Path_1" data-name="Path 1" d="M141.314,136.63l1.055-.085a.568.568,0,0,0,.52-.61l-.129-1.58a.565.565,0,0,0-.609-.519l-2.354,0-2.549-5.724a2.074,2.074,0,0,0-2.032-1.116h-15a2.084,2.084,0,0,0-2.036,1.116l-2.546,5.724-2.354,0a.568.568,0,0,0-.61.519l-.127,1.58a.567.567,0,0,0,.519.61l1.055.085a10.131,10.131,0,0,0-1.833,5.852l.238,3.025a1.649,1.649,0,0,0,.9,1.355v1.6c.1,2.185.788,2.185,2.319,2.185s2.32,0,2.423-2.185v-1.417l9.551,0,9.468,0v1.415c.1,2.185.787,2.185,2.319,2.185s2.32,0,2.422-2.185v-1.6a1.734,1.734,0,0,0,.978-1.355l.242-3.025A10.131,10.131,0,0,0,141.314,136.63ZM122.257,143.5a.568.568,0,0,1-.566.567h-5.577a.567.567,0,0,1-.568-.567v-2.04a.565.565,0,0,1,.568-.567l5.577.453a.568.568,0,0,1,.566.566Zm-4.9-7.98,2.742-6.307h15.232l2.741,6.307H117.359Zm22.53,7.98a.567.567,0,0,1-.567.567h-5.577a.568.568,0,0,1-.567-.567v-1.588a.569.569,0,0,1,.567-.566l5.577-.453a.565.565,0,0,1,.567.567Z" transform="translate(-112.289 -126.994)"></path> </g></svg>
                                        </span>
                            ...
                            <label class="cursor-pointer">
                                ...
                                <div
                                    class="border-2 border-gray-200 rounded-xl p-3 peer-checked:border-gray-500 peer-checked:bg-gray-50 transition md:p-4 md:hover:border-gray-400">
                                    <div class="flex items-center gap-2">
                                        <span class="w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center shrink-0">
                    ...
                    {{-- NOMINAL & CATATAN --}}
                    <div class="md:bg-white md:rounded-[2rem] md:shadow-sm md:border md:border-gray-100 md:p-6 space-y-4 md:space-y-6">
                        <div>
                ...
                <div class="md:col-span-5 md:space-y-6">

                    {{-- OPSI AUTO RECEIPT (KHUSUS PARKIR) --}}
                    <div id="auto-receipt-section" class="mb-6 hidden md:mb-0">
                        <label class="flex items-center space-x-3 p-3 bg-blue-50 border border-blue-100 rounded-xl cursor-pointer md:rounded-2xl">
                    ...
                    <div id="parking-details-section" class="hidden space-y-4">
                        <div class="md:bg-white md:rounded-[2rem] md:shadow-sm md:border md:border-gray-100 md:p-6">
                            <div class="p-4 bg-gray-50 border border-gray-200 rounded-xl md:bg-gray-50">
                    ...
                    {{-- FOTO STRUK --}}
                    <div class="mb-6 md:mb-0 md:bg-white md:rounded-[2rem] md:shadow-sm md:border md:border-gray-100 md:p-6" id="photo-section">
                    ...
                    <div class="flex gap-3 mb-24 md:mb-0">
                        <a href="{{ route('sales.history') }}"
                            class="flex-1 bg-gray-200 text-gray-700 py-4 rounded-xl font-bold text-sm text-center md:flex-none md:px-8 md:hover:bg-gray-300 md:transition">Batal</a>
                        <button type="submit" id="btn-submit"
                            class="flex-[2] bg-blue-600 text-white py-4 rounded-xl font-bold text-sm shadow-lg disabled:bg-gray-400 disabled:cursor-not-allowed md:flex-1 md:hover:bg-blue-700 md:transition">Simpan</button>
                    </div>
                </div>
            </div>
        </form>
