...
@section('content')
    <div class="px-5 md:px-8 py-6 md:py-8">
        ...
        <form id="bulkApproveForm" action="{{ route(auth()->user()->role . '.reimburse.bulk_approve') }}" method="POST" class="hidden">
            @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 pb-24">
            @forelse($groups as $items)
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 hover:shadow-md transition flex flex-col">
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex justify-between items-start gap-2">
                                ...
                                <span class="bg-yellow-100 text-yellow-700 text-[10px] font-bold px-2 py-1 rounded-full uppercase whitespace-nowrap">
                                    @if(auth()->user()->isSupervisor()) Menunggu SPV @else Menunggu HRD @endif
                                </span>
                            </div>
                    ...
                    {{-- RIWAYAT APPROVAL (untuk HRD: sudah lewat SPV) --}}
                    @if($isHrd && $first->approved_by_spv_at)
                        <div class="bg-green-50 border border-green-200 rounded-lg p-2 mb-3 text-xs">
                            <p class="text-green-700"><svg class="w-4 h-4 inline mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg> Disetujui SPV: {{ $first->approvedBySpv?->name ?? '-' }}
                                ({{ \Carbon\Carbon::parse($first->approved_by_spv_at)->format('d M H:i') }})</p>
                        </div>
                    @endif
    ...
    <div id="reject-modal" class="hidden modal-overlay fixed inset-0 z-[60] flex items-center justify-center bg-black/50" onclick="if(event.target===this) closeReject()">
        <div class="modal-box bg-white rounded-2xl shadow-xl w-[90%] max-w-md overflow-hidden">
            <form id="bulkRejectForm" action="{{ route(auth()->user()->role . '.reimburse.bulk_reject') }}" method="POST">
                @csrf
                <input type="hidden" name="expense_ids" value="">
                <div class="p-6">
