    ...
    <div class="px-5 py-6">
        ...
        <form action="{{ route('hrd.reset.password', $user->id) }}" method="POST" class="mb-6">
            ...
            <button type="submit" onclick="return confirm('Reset password untuk {{ $user->name }}?')"
                class="w-full bg-yellow-500 text-white py-3 rounded-xl font-bold text-sm flex items-center justify-center gap-2">
        ...
        <form action="{{ route('hrd.user.toggle_status', $user->id) }}" method="POST" class="mb-6">
            ...
            @if($user->is_active)
                <button type="submit"
                    ...
                    class="w-full bg-red-600 text-white py-3 rounded-xl font-bold text-sm flex items-center justify-center gap-2">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            @else
                <button type="submit" onclick="return confirm('Aktifkan kembali akun {{ $user->name }}?')"
                    class="w-full bg-green-600 text-white py-3 rounded-xl font-bold text-sm flex items-center justify-center gap-2">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <a href="{{ route('sales.history.detail', $log->id) }}" class="block">
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 hover:shadow-md transition">
                        <div class="flex justify-between items-center mb-3 pb-3 border-b border-gray-100">
                            <div>
                                <h3 class="font-bold text-gray-800">{{ \Carbon\Carbon::parse($log->date)->format('d M Y') }}
                                </h3>
                            ...
                            @foreach($log->visits as $visit)
                                <div class="flex items-start gap-3 p-2 bg-gray-50 rounded-lg">
                                    <div
                                        ...
                                        @if($visit->status == 'completed') <svg class="w-4 h-4" fill="none" stroke="currentColor"
                                                viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                    d="M5 13l4 4L19 7"></path>
                                            </svg>
                                        @elseif($visit->status == 'failed') <svg class="w-4 h-4" fill="none" stroke="currentColor"
                                                viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
