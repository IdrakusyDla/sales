    ...
    <div class="px-5 md:px-8 py-6 md:py-8">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
            <div class="flex items-center gap-3">
                <a href="{{ route('hrd.dashboard') }}" class="text-gray-600 hover:text-gray-800 transition">
        ...
        {{-- TAB SWITCHER BAR --}}
        <div class="flex bg-white rounded-2xl p-1.5 gap-1.5 mb-6 border border-gray-200/60 shadow-sm">
            <button id="tab-btn-activity" onclick="switchTab('activity')" 
                class="flex-1 py-3 rounded-xl flex items-center justify-center gap-2 font-bold text-sm transition-all focus:outline-none duration-200">
        ...
        <div id="tab-content-activity" class="space-y-6">
            ...
            <div class="bg-blue-50/50 rounded-2xl p-5 border border-blue-100">
                <form method="GET" action="{{ route('hrd.show.user', $user->id) }}" class="filter-form flex flex-col md:flex-row md:items-end gap-4">
                    ...
                    <div class="flex gap-2 w-full md:w-auto md:shrink-0">
                        ...
                        @if(request('start_date') || request('end_date'))
                            <a href="{{ route('hrd.show.user', $user->id) }}" class="bg-white hover:bg-gray-50 border border-gray-200 transition text-gray-600 px-5 py-3 md:py-2.5 md:px-5 rounded-xl font-bold text-sm shadow-sm flex items-center justify-center">
                                Reset
                            </a>
                        @endif
                    </div>
        ...
        {{-- TAB CONTENT: PENGATURAN & AKSES --}}
        <div id="tab-content-settings" class="hidden">
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {{-- KOLOM KIRI: EDIT DATA KARYAWAN --}}
                <div class="space-y-6">
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
                        <h3 class="font-bold text-gray-800 mb-4 text-sm border-b border-gray-100 pb-2 flex items-center gap-2">
                            <svg class="w-4 h-4 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"></path>
                            </svg>
                            Data Perusahaan & Jabatan
                        </h3>
                        <form action="{{ route('hrd.user.update_profile', $user->id) }}" method="POST" class="space-y-4">
                            @csrf
                            <div>
                                <label class="block text-xs font-bold text-gray-700 mb-1.5">Perusahaan</label>
                                <select name="company_id" class="w-full border border-gray-300 rounded-xl p-3 text-sm focus:ring-2 focus:ring-blue-600 focus:outline-none">
                            ...
                            <div>
                                <label class="block text-xs font-bold text-gray-700 mb-1.5">Jabatan</label>
                                <select name="job_position_id" class="w-full border border-gray-300 rounded-xl p-3 text-sm focus:ring-2 focus:ring-blue-600 focus:outline-none">
                                    <option value="">-- Tidak Dipilih --</option>
                            ...
                            <div>
                                <label class="flex items-start gap-3 p-4 border border-gray-200 rounded-xl cursor-pointer hover:border-blue-500 transition">
                                    <input type="checkbox" name="fuel_reimbursement_enabled" value="1" {{ $user->fuel_reimbursement_enabled ? 'checked' : '' }} class="w-5 h-5 text-blue-600 rounded mt-0.5">
                                    <span class="flex-1">
                                        <span class="font-bold text-sm block text-gray-800">Reimburse Bahan Bakar</span>
                                        <span class="text-xs text-gray-500 block mt-0.5">Jika dinonaktifkan, karyawan tidak perlu input odometer & tidak dapat reimburse bensin otomatis</span>
                                    </span>
                                </label>
                            </div>
                            <button type="submit" class="w-full bg-blue-600 text-white px-6 py-3 rounded-xl font-bold text-sm hover:bg-blue-700 transition shadow-sm">
                                Simpan Perubahan
                            </button>
                        </form>
                    </div>
                ...
                {{-- KOLOM KANAN: SUPERVISOR & AKSI ADMINISTRASI --}}
                <div class="space-y-6">
                    ...
                    @if($user->role === 'sales')
                        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
                            ...
                            <div class="space-y-2 mb-4">
                                    ...
                                    <div class="flex justify-between items-center bg-gray-50 p-3 rounded-xl border border-gray-100 shadow-sm">
                                        ...
                                        <form action="{{ route('hrd.remove.supervisor', $user->id) }}" method="POST">
                                            ...
                                            <button type="submit" onclick="return confirm('Hapus {{ $spv->name }} dari supervisor sales ini?')"
                                                class="text-red-500 hover:text-red-700 hover:bg-red-50 transition px-3 py-1 rounded-lg text-xs font-bold">
                                                Hapus
                                            </button>
                                        </form>
                                    </div>
                                @empty
                                    <div class="bg-gray-50 border border-gray-100 rounded-xl p-4 text-center">
                                        <p class="text-xs text-gray-500 italic">Belum ada supervisor yang ditugaskan</p>
                                    </div>
                                @endforelse
                            </div>
                            ...
                            <form action="{{ route('hrd.add.supervisor', $user->id) }}" method="POST" class="flex gap-2">
                                @csrf
                                <select name="supervisor_id" class="flex-1 text-sm border border-gray-300 rounded-xl p-3 block w-full bg-white focus:ring-2 focus:ring-indigo-600 focus:outline-none" required>
                                    <option value="">-- Pilih Akses Supervisor Baru --</option>
                                ...
                                <button type="submit" class="bg-indigo-600 text-white px-5 py-3 rounded-xl text-sm font-bold hover:bg-indigo-700 shadow-sm transition shrink-0">
                                    Tambah
                                </button>
                            </form>
                        </div>
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            ...
                            <form action="{{ route('hrd.reset.password', $user->id) }}" method="POST">
                                @csrf
                            ...
                            <form action="{{ route('hrd.user.toggle_status', $user->id) }}" method="POST">
                                @csrf
                                @if($user->is_active)
                                    <button type="submit"
...
<script>
    function switchTab(tabId) {
        ...
        if (!btnActivity || !btnSettings || !contentActivity || !contentSettings) return;

        // Reset classes for tab buttons
        btnActivity.className = "flex-1 py-3 rounded-xl flex items-center justify-center gap-2 font-bold text-sm transition-all focus:outline-none duration-200";
        btnSettings.className = "flex-1 py-3 rounded-xl flex items-center justify-center gap-2 font-bold text-sm transition-all focus:outline-none duration-200";

        if (tabId === 'activity') {
            btnActivity.classList.add('bg-blue-600', 'text-white', 'shadow-sm');
            btnSettings.classList.add('text-gray-600', 'hover:bg-gray-50');
            contentActivity.classList.remove('hidden');
            contentSettings.classList.add('hidden');
        } else {
            btnSettings.classList.add('bg-blue-600', 'text-white', 'shadow-sm');
            btnActivity.classList.add('text-gray-600', 'hover:bg-gray-50');
            contentSettings.classList.remove('hidden');
            contentActivity.classList.add('hidden');
        }

        // Save active tab state in localStorage
        localStorage.setItem('hrd_show_user_active_tab', tabId);
    }

    document.addEventListener('DOMContentLoaded', function() {
        const savedTab = localStorage.getItem('hrd_show_user_active_tab') || 'activity';
        switchTab(savedTab);
    });
</script>
@endsection