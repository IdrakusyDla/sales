        ...
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
            <form action="{{ route('hrd.export.excel') }}" method="GET">
                ...
                <div class="flex justify-center mb-6">
                    <div class="w-20 h-20 bg-green-50 rounded-full flex items-center justify-center text-green-600 border border-green-100">
                ...
                <div class="grid grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <input type="date" name="start_date" value="{{ date('Y-m-01') }}" class="w-full border border-gray-300 rounded-xl p-3 text-sm focus:ring-indigo-500">
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-gray-500 uppercase mb-2">Sampai Tanggal</label>
                        <input type="date" name="end_date" value="{{ date('Y-m-d') }}" class="w-full border border-gray-300 rounded-xl p-3 text-sm focus:ring-indigo-500">
                    </div>
                </div>
                ...
                <div class="mb-6">
                    ...
                    <select name="user_id" class="w-full border border-gray-300 rounded-xl p-3 text-sm focus:ring-indigo-500 bg-white">
...
<div class="hidden md:block px-8 py-8 min-h-screen bg-slate-50/50">
    ...
    <div class="mb-8">
        ...
        <p class="text-gray-500 font-medium">Unduh rekap absensi dan reimburse ke Excel (.xlsx)</p>
    </div>
    ...
    <div class="grid grid-cols-12 gap-8 items-start">
                    ...
                    <form action="{{ route('hrd.export.excel') }}" method="GET">
                        ...
                        <div class="grid grid-cols-2 gap-6 mb-6">
                            <div>
                                ...
                                <input type="date" name="start_date" value="{{ date('Y-m-01') }}" class="w-full border-gray-200 bg-gray-50 text-gray-800 rounded-xl p-4 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-shadow">
                            </div>
                            <div>
                                <label class="block text-sm font-bold text-gray-700 mb-2 uppercase tracking-wider">Sampai Tanggal</label>
                                <input type="date" name="end_date" value="{{ date('Y-m-d') }}" class="w-full border-gray-200 bg-gray-50 text-gray-800 rounded-xl p-4 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-shadow">
                            </div>
                        </div>

                        {{-- 3. FILTER SALES --}}
                        <div class="mb-8">
                            <label class="block text-sm font-bold text-gray-700 mb-2 uppercase tracking-wider">Filter Sales (Opsional)</label>
                            <select name="user_id" class="w-full border-gray-200 bg-gray-50 text-gray-800 rounded-xl p-4 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-shadow">
                                <option value="">-- Download Semua Sales --</option>
                                @foreach ($users as $u)
                                    <option value="{{ $u->id }}">{{ $u->name }}</option>
                                @endforeach
        ...
        <div class="col-span-4 space-y-6 sticky top-8">
            ...
            <div class="bg-white rounded-[2rem] shadow-sm border border-gray-100 p-8">
                <div class="w-14 h-14 bg-green-50 rounded-2xl flex items-center justify-center text-green-600 mb-4">
            ...
            {{-- Panduan --}}
            <div class="bg-indigo-50 rounded-[2rem] border border-indigo-100 p-8">
                <div class="w-14 h-14 bg-indigo-100 rounded-2xl flex items-center justify-center text-indigo-600 mb-4">
                ...
                <h3 class="font-extrabold text-indigo-900 text-lg mb-3">Panduan</h3>
                <ul class="space-y-2 text-sm text-indigo-700">
                    ...
                    <li class="flex items-start gap-2"><span class="font-bold">3.</span> Opsional: filter per sales</li>
